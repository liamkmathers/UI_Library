...
export function createFluidSimulation(canvas: HTMLCanvasElement, _config?: Partial<Config>) {
  ...
  function render() {
    ...
    if (config.TRANSPARENT) {
      ...
      context.fillStyle = `rgba(${Math.floor(config.BACK_COLOR[0] * 255)}, ${Math.floor(config.BACK_COLOR[1] * 255)}, ${Math.floor(config.BACK_COLOR[2] * 255)}, ${config.BACK_COLOR[3]})`;
  ...
  function onPointerEnter(e: MouseEvent | TouchEvent) {
  ...
  return () => {
    ...
    canvas.removeEventListener('touchstart', onPointerEnter);
